#!/usr/bin/env bash
# Offline tests for .claude/hooks/code-review-graph-update.sh (issue #360).
#
# Hermetic: a PATH canary stands in for the real `code-review-graph` binary
# so no real graph write ever happens; the decision log is redirected via
# CLAUDE_HOOK_LOG.
#
# Usage: bash .claude/hooks/__tests__/code-review-graph-update.test.sh

set -u

HERE="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
HOOK="$HERE/../code-review-graph-update.sh"
...
# TMPDIR_TEST is not a git repo -> fail-open skip
[[ "$rc" == "0" && "$(last_decision "$log")" == "skip" ]] \
...
rc="$(env "CLAUDE_HOOK_LOG=$log" "PATH=$TMPDIR_TEST/bin:$PATH" bash "$HOOK" </dev/null >/dev/null 2>&1; echo $?)"
[[ "$rc" == "0" && "$(last_decision "$log")" == "updated" && -s "$CANARY_FILE" ]] \
  && report PASS 'tool present, in git repo -> updated, invoked once' \
  || report FAIL 'tool present, in git repo -> updated, invoked once' "rc=$rc decision=$(last_decision "$log") canary=$(cat "$CANARY_FILE" 2>/dev/null)"
