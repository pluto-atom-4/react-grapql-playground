...
describe('Upload Routes', () => {
  ...
  it('POST /upload - should upload file and return fileId', async () => {
    ...
    const res = await request(app).post('/upload').attach('file', testFile);
  ...
  it('POST /upload - should return 400 if no file provided', async () => {
    const res = await request(app).post('/upload').send({ buildId: 'build-123' });
  ...
  it('GET /files/:fileId - should serve uploaded file', async () => {
    ...
    // Upload file
    const uploadRes = await request(app).post('/upload').attach('file', testFile);
    ...
    const serveRes = await request(app).get(`/files/${uploadedFileName}`);
  ...
  it('GET /files/:fileId - should return 404 for non-existent file', async () => {
    const res = await request(app).get('/files/non-existent-file-xyz-12345.txt');
  ...
  it('POST /upload - should reject files exceeding 50MB size limit', async () => {
    ...
    try {
      ...
      const res = await request(app).post('/upload').attach('file', result);
